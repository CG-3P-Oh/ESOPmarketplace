...
class DIPL_DiviPlus extends DiviExtension {
	...
	public function __construct( $name = 'divi-plus', $args = array() ) {
		...
		// Moved outside, to use styles globally.
		...
		add_action( 'wp_ajax_dipl_stripe_generate_session', array( $this, 'dipl_stripe_generate_session' ) );
		add_action( 'wp_ajax_nopriv_dipl_stripe_generate_session', array( $this, 'dipl_stripe_generate_session' ) );

		add_action( 'wp_ajax_dipl_get_blog_posts', array( $this, 'dipl_get_blog_posts' ) );
		add_action( 'wp_ajax_nopriv_dipl_get_blog_posts', array( $this, 'dipl_get_blog_posts' ) );
		...
		// Add post category image field.
		if ( is_admin() && $this->dipl_is_module_enable( 'blog_categories' ) ) {
			add_action( 'category_add_form_fields', array( $this, 'dipl_add_post_category_image_field' ), 10 );
			add_action( 'category_edit_form_fields', array( $this, 'dipl_edit_post_category_image_field' ), 10, 2 );
			add_action( 'created_category', array( $this, 'dipl_save_post_category_image' ), 10, 2 );
			add_action( 'edited_category', array( $this, 'dipl_save_post_category_image' ), 10, 2 );
			add_action( 'admin_enqueue_scripts', array( $this, 'dipl_blog_category_enqueue_scripts' ) );

			// Add columns.
			add_filter( 'manage_edit-category_columns', array( $this, 'dipl_category_image_columns' ) );
			add_filter( 'manage_category_custom_column', array( $this, 'dipl_category_image_column' ), 10, 3 );
		}
	}
	...
	public function dipl_register_scripts() {
		...
		wp_register_script( 'elicus-popper-script', "{$this->plugin_dir_url}includes/assets/js/popper.min.js", array('jquery'), '2.11.8', true );
		wp_register_script( 'elicus-tippy-script', "{$this->plugin_dir_url}includes/assets/js/tippy-bundle.umd.min.js", array('elicus-popper-script'), '6.3.7', true );

		// Text scroll animation GSAP 3.12.2
		wp_register_script( 'elicus-scroll-trigger-script', "{$this->plugin_dir_url}includes/assets/js/ScrollTrigger.min.js", array('jquery'), '3.12.2', true );
		wp_register_script( 'elicus-gsap-script', "{$this->plugin_dir_url}includes/assets/js/gsap.min.js", array('elicus-scroll-trigger-script'), '3.12.2', true );


		// Used on sticky video module.
		wp_register_script( 'elicus-youtube-js-api-script', "https://www.youtube.com/iframe_api", array('jquery'), $this->version, true );
		wp_register_script( 'elicus-vimeo-js-api-script', "https://player.vimeo.com/api/player.js", array('jquery'), $this->version, true );

		// JPlayer
		wp_register_script( 'elicus-jplayer-script', "{$this->plugin_dir_url}includes/assets/js/jquery.jplayer.min.js", array('jquery', 'jquery-ui-core'), '2.9.2', true );

		// Chart.js, chartjs.org
		wp_register_script( 'elicus-chartjs-script', "{$this->plugin_dir_url}includes/assets/js/chartjs.min.js", array('jquery'), '4.5.1', true );
	}
	...
	public function dipl_admin_enqueue_scripts( $hook_suffix ) {
		if ( 'post-new.php' === $hook_suffix || 'post.php' === $hook_suffix || 'edit-tags.php' === $hook_suffix ) {
	...
	public function dipl_check_filetype_and_ext( $types, $file, $filename, $mimes ) {
		...
		if ( 'pdf' === $check_file['ext'] ) {
			$types['ext']  = 'pdf';
			$types['type'] = 'application/pdf';
		}
		if ( 'svg' === $check_file['ext'] ) {
			$data['ext']  = 'svg';
	 ...
	 */
	static function dipl_get_plugin_options() {
		if ( empty( self::$plugin_options ) ) {
			self::$plugin_options = get_option( ELICUS_DIVI_PLUS_OPTION );
		}
	 ...
	 * @return boolean
	 */
	public function dipl_is_module_enable( $shortcode ) {
		$plugin_options = $this->dipl_get_plugin_options();
		
		// If not active modules in options return true.
		if ( ! isset( $plugin_options['dipl-modules'] ) ) {
			return true;
		}

		// Get the active modules, check if enabled.
		$modules = explode( ',', $plugin_options['dipl-modules'] );
		if ( in_array( 'dipl_' . $shortcode, $modules ) ) {
			return true;
		}
	...
	public function dipl_ajax_search_results() {
		...
		$exclude_protected  = isset( $_POST['exclude_protected'] ) ? trim( sanitize_text_field( wp_unslash( $_POST['exclude_protected'] ) ) ) : 'off';
		...
		if ( 'on' === $exclude_protected ) {
			$where .= " AND ( $wpdb->posts.post_password IS NULL OR $wpdb->posts.post_password = '' )";
		}
		if ( in_array('sitepress-multilingual-cms/sitepress.php', apply_filters('active_plugins', get_option('active_plugins') ) ) ) { 
	...
	public function dipl_get_team_members() {
		...
		$defaults = array(
			...
			'tax_query_field'                     => 'term_id',
			'use_button'                          => 'off',
			'button_text'                         => esc_html__( 'Read more', 'divi-plus' ),
			'custom_link_button'                  => 'off',
			'link_button_icon'                    => '',
			'link_target'                         => 'off',
		);
		...
		foreach ( $defaults as $key => $default ) {
			// phpcs:ignore ET.Sniffs.ValidatedSanitizedInput.InputNotSanitized
			$raw_value = isset( $_POST['props'][$key] ) ? $_POST['props'][$key] : null;
			if ( isset( $raw_value ) && et_()->includes( $raw_value, '%' ) ) {
				// phpcs:ignore ET.Sniffs.ValidatedSanitizedInput.InputNotSanitized
				$prepared_value  = preg_replace( '/%([a-f0-9]{2})/', '%_$1', $raw_value );
				${$key} = preg_replace( '/%_([a-f0-9]{2})/', '%$1', trim( sanitize_text_field( wp_unslash( $prepared_value ) ) ) );
			} else {
				// phpcs:ignore ET.Sniffs.ValidatedSanitizedInput.InputNotSanitized
				$raw_value = et_()->array_get( $_POST['props'], $key, $default );
				${$key} = trim( sanitize_text_field( wp_unslash( $raw_value ) ) );
			}
		}

		$valid_heading_tag = array( 'h1', 'h2', 'h3', 'h4', 'h5', 'h6' );

		$processed_name_level = esc_html( $name_level );
		...
		if ( $query->have_posts() ) {
			...
			$common_attrs   = array();
			$common_attrs[] = 'data-close_icon_position="' . $close_icon_position . '"';
			$common_attrs[] = 'data-popup_name_level="' . $processed_popup_name_level . '"';

			if ( '' !== $popup_elements ) { 
				if ( in_array( 'image', $popup_elements ) ) {
					$common_attrs[] = 'data-image="on"';
					$common_attrs[] = 'data-image_size="'.$image_size.'"';
				}
				if ( in_array( 'designation', $popup_elements ) ) { 
					$common_attrs[] = 'data-designation="on"';
				}
				if ( in_array( 'content', $popup_elements ) ) { 
					$common_attrs[] = 'data-content="on"';  
				}
				if ( in_array( 'skills_bars', $popup_elements ) ) { 
					$common_attrs[] = 'data-skills_bars="on"';
					if ( 'layout1' === $bar_layout ) {
						$common_attrs[] = 'data-bar_layout="1"';
					} else {
						$common_attrs[] = 'data-bar_layout="2"';
					}   
				}
				if ( in_array( 'social_icons', $popup_elements ) ) { 
					$common_attrs[] = 'data-social_icons="on"'; 
				}
			}

			$wrapper_class = array( 'dipl_team_grid_item', 'dipl_team_member_grid_wrapper' );
			if ( 'open_popup' === $onclick_trigger && 'on' !== $use_button ) {
				$wrapper_class[] = 'dipl_team_popup';
			}
			if ( 'open_link' === $onclick_trigger && 'on' !== $use_button ) {
				$wrapper_class[] = 'dipl_team_link';
			}
			...
			while ( $query->have_posts() ) {
				...
				$data_attrs        = $common_attrs;

				$data_attrs[] = 'data-id="' . esc_attr( $post_id ) . '"';
				...
				$readmore_button = '';
				if ( 'do_nothing' !== $onclick_trigger && 'on' === $use_button ) {
					$button_class = array( 'dipl-team-item-link-btn' );
					$button_link  = esc_url( get_permalink( $post_id ) );
					if ( 'open_popup' === $onclick_trigger ) {
						$button_class[] = 'dipl_team_popup';
						$button_link    = '#team-member-' . esc_attr( $post_id );
					}
					$readmore_button = dipl_render_divi_button( array(
						'button_text'         => esc_attr( $button_text ),
						'button_text_escaped' => true,
						'has_wrapper'      	  => false,
						'button_classname'    => $button_class,
						'button_url'          => $button_link,
						'url_new_window'      => esc_attr( $link_target ),
						'button_custom'       => esc_attr( $custom_link_button ),
						'custom_icon'         => et_core_esc_previously( $link_button_icon ),
					) );
					if ( ! empty( $readmore_button ) ) {
						$readmore_button = sprintf(
							'<div class="et_pb_button_wrapper dipl-team-item-link-btn-wrap">%1$s</div>',
				...
				if ( 'open_link' === $onclick_trigger ) {
					$data_attrs[] = sprintf( 'data-link="%1$s"', get_permalink( $post_id ) );
					$data_attrs[] = sprintf( 'data-link_target="%1$s"', ( 'on' === $link_target ) ? '_blank' : '_self' );
				} elseif ( 'open_popup' === $onclick_trigger ) {
					$data_attrs[] = 'data-id="' . esc_attr( $post_id ) . '"';
				}
	...
	public function dipl_stripe_generate_session() {
		if ( ! isset( $_POST['dipl_stripe_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['dipl_stripe_nonce'] ) ), 'elicus-divi-plus-nonce' ) ) {
			return;
		}
		if ( ! isset( $_POST['props'] ) || '' === $_POST['props'] ) {
			return;
		...
		// Load stripe class file.
		require_once( ELICUS_DIVI_PLUS_DIR_PATH . 'includes/integrations/class-dipl-stripe.php' );

		// Create instance.
		$stripe = DIPL_Stripe::instance();

		// Get the args to pass.
		$args = array(
			'payment_mode'  => sanitize_text_field( $payment_mode ),
			'product_name'  => sanitize_text_field( $product_name ),
			'product_price' => floatval( $product_price ),
			'quantity'      => intval( $product_qty ),
			'success_url'   => esc_url( $success_url ),
			'cancel_url'    => esc_url( $cancel_url ),
		);
		$response = $stripe->generate_checkout_session_url( $args );
		...
		wp_send_json_success( [ 'session_url' => $response ] );
		exit;
	}

	public function dipl_get_blog_posts() {
		if ( ! isset( $_POST['dipl_get_blog_posts_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['dipl_get_blog_posts_nonce'] ) ), 'elicus-divi-plus-blog-nonce' ) ) {
			return;
		}
		if ( ! isset( $_POST['props'] ) ) {
			return;
		...
		$defaults = array(
			...
			'read_more_icon'         => '',
			'read_more_icon_desktop' => '',
			'read_more_icon_tablet'  => '',
			'read_more_icon_phone'   => '',
			'title_level'            => 'h4',
			'is_user_logged_in'      => false,
			'is_single'              => false,
			'current_post_id'        => 0,
		...
		$posts_number = ( 0 === $posts_number ) ? -1 : (int) $posts_number;
		...
		if ( is_single() && ! isset( $args['post__not_in'] ) ) {
			$args['post__not_in'] = array( intval( $current_post_id ) );
		}
		if ( 'on' !== $show_content ) {
			$excerpt_length = ( '' === $excerpt_length ) ? 270 : intval( $excerpt_length );
		}
		if ( 'on' === $show_read_more ) {
			$read_more_text = ( ! isset( $read_more_text ) || '' === $read_more_text ) ? esc_html__( 'Read More', 'divi-plus' ) :
			sprintf( esc_html__( '%s', 'divi-plus' ), esc_html( $read_more_text ) );
		}

		$read_more_icon = array(
			'desktop' => $read_more_icon_desktop,
			'tablet'  => $read_more_icon_tablet,
			'phone'   => $read_more_icon_phone
		);
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				$post_classes   = array_map( 'sanitize_html_class', get_post_class( 'dipl_blog_post dipl_blog_post_page_' . intval( $page ) . $no_thumb_class ) );
				...
				$read_more_icon_array = array(
					...
					'custom_icon'         => et_core_esc_previously( $read_more_icon['desktop'] ),
					'has_wrapper'         => false,
				);
				...
				$read_more_button = dipl_render_divi_button( $read_more_icon_array );
				...
				if ( file_exists( get_stylesheet_directory() . '/divi-plus/layouts/blog/' . sanitize_file_name( $layout ) . '.php' ) ) {
					...
					include $this->plugin_dir . 'modules/Blog/layouts/' . sanitize_file_name( $layout ) . '.php';
				}
			}
