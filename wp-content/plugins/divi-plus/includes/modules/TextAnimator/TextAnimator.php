...
class DIPL_TextAnimator extends ET_Builder_Module {
	...
	public function get_fields() {
		...
		$dipl_animator_fields = array(
			...
			'animate_only_once'  => array(
				'label'           => esc_html__( 'Animate Only Once', 'divi-plus' ),
				'type'            => 'yes_no_button',
				'option_category' => 'configuration',
				'options'         => array(
					'off' => esc_html__( 'No', 'divi-plus' ),
				...
				'tab_slug'        => 'general',
				'toggle_slug'     => 'animation',
				'description'     => esc_html__( 'Here you can choose to animate text only once, it will stop automatically after all texts animated.', 'divi-plus' ),
			),
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		$animate_only_once             = $this->props['animate_only_once'];

		wp_enqueue_script( 'dipl-text-animator-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TextAnimator/dipl-text-animator-custom.min.js', array('jquery'), '1.0.1', true );
		
		$file = et_is_builder_plugin_active() ? 'style-dbp' : 'style';
        wp_enqueue_style( 'dipl-text-animator-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TextAnimator/' . $file . '.min.css', array(), '1.0.0' );
		...
		switch ( $animation_layout ) {
			case 'typing':
				$animated_phrase = '<span
					...
					data-animate-only-once="' . esc_attr( $animate_only_once ) . '"
				></span>';
			break;
			default:
				ET_Builder_Element::set_style( $render_slug, array(
