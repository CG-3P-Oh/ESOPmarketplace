...
class DIPL_PriceListItem extends ET_Builder_Module {
	...
	public function get_fields() {
		$fields = array(
			...
			'price_period'            => array(
				'label'           => esc_html__( 'Item Price Period', 'divi-plus' ),
				'type'            => 'text',
				'option_category' => 'basic_option',
				'default'         => '',
				'tab_slug'        => 'general',
				'toggle_slug'     => 'main_content',
				'description'     => esc_html__( 'Here you can input the item price period like: day, week, month, year etc.', 'divi-plus' ),
			),
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		$price_period          = esc_attr( $this->props['price_period'] );
		...
		if ( 'use_icon' === $item_thumbnail_option ) {
			...
			et_pb_responsive_options()->generate_responsive_css( $icon_font_size, "{$this->main_css_element} .dipl_price_list_icon", 'font-size', $render_slug, '!important;', 'range' );
			
			$icon_color = et_pb_responsive_options()->get_property_values( $this->props, 'icon_color' );
			et_pb_responsive_options()->generate_responsive_css( $icon_color, "{$this->main_css_element} .dipl_price_list_icon", 'color', $render_slug, '!important;', 'color' );
        ...
        wp_enqueue_style( 'dipl-price-list-item-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/PriceListItem/' . $file . '.min.css', array(), '1.0.0' );
