 ...
 *
 * This template can be overridden by copying it to yourtheme/divi-plus/layouts/blog-slider/layout3.php
 *
...
$output .= sprintf(
	'<article id="post-' . $post_id . '" class="%1$s ' . $post_classes . '" >',
...
if ( 'on' === $show_date || 'on' === $show_author || 'on' === $show_comments ) {
	$output .= sprintf(
		'<p class="dipl_blog_slider_meta">%1$s%2$s%3$s%4$s%5$s</p>',
		(
			'on' === $show_author ?
			et_get_safe_localization(
				sprintf(
