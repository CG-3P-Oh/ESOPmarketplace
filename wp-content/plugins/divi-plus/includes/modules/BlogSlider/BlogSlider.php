...
class DIPL_BlogSlider extends ET_Builder_Module {
		...
		return array(
			...
			'borders' => array(
				'image' => array(
					...
					'css' => array(
						'main' => array(
							'border_radii'  => '%%order_class%% .dipl_blog_slider_post_image',
							'border_styles' => '%%order_class%% .dipl_blog_slider_post_image',
						),
						'important' => 'all',
					),
					'tab_slug'    => 'advanced',
	...
	public function get_fields() {
		return array_merge(
			array(
				...
				'post_content_vertical_align' => array(
	...
	public static function get_blog_posts( $attrs = array(), $conditional_tags = array(), $current_page = array() ) {
		...
		if ( $query->have_posts() ) {
			while ( $query->have_posts() ) {
				...
				if ( file_exists( get_stylesheet_directory() . '/divi-plus/layouts/blog-slider/' . sanitize_file_name( $slider_layout ) . '.php' ) ) {
					include get_stylesheet_directory() . '/divi-plus/layouts/blog-slider/' . sanitize_file_name( $slider_layout ) . '.php';
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		if ( $query->have_posts() ) {
        	...
        	wp_enqueue_style( 'dipl-blog-slider-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/BlogSlider/' . $file . '.min.css', array(), '1.0.0' );

			if ( ! empty( array_filter( $meta_icon_font_size ) ) ) {
				et_pb_responsive_options()->generate_responsive_css( $meta_icon_font_size, '%%order_class%% .dipl_blog_slider_meta .et-pb-icon', 'font-size', $render_slug, '!important;', 'type' );
			}
			...
			if ( ! empty( array_filter( $featured_image_height ) ) ) {
				et_pb_responsive_options()->generate_responsive_css( $featured_image_height, '%%order_class%% .layout3 .dipl_blog_slider_image_wrapper', 'height', $render_slug, '!important;', 'type' );
			}
			...
			if ( '' !== $category_background_color ) {
				...
				self::set_style(
					...
					array(
						'selector'    => "{$this->main_css_element} .dipl_blog_slider_post_categories a",
						'declaration' => 'margin: 0 2px 5px 0 !important;',
					)
				);
			}
			...
			if ( 'on' === $equalize_posts_height ) {
				self::set_style(
					...
					array(
						...
						'declaration' => 'align-items: stretch !important;',
					)
				);
				self::set_style(
					$render_slug,
					array(
						'selector'    => '%%order_class%% .dipl_swiper_inner_wrap .swiper-slide',
				...
				self::set_style(
					$render_slug,
					array(
						'selector'    => '%%order_class%% .dipl_swiper_inner_wrap .dipl_blog_slider_post',
				...
				self::set_style(
					$render_slug,
					array(
						'selector'    => '%%order_class%% .dipl_swiper_inner_wrap .dipl_blog_slider_read_more_link',
						'declaration' => 'display: flex; align-items: flex-end; flex-grow: 1;',
					)
				);
			...
			while ( $query->have_posts() ) {
				...
				$output .= '<div class="dipl_blog_post_slide swiper-slide">';
			...
			if ( 'on' === $show_arrow ) {
				...
				if ( ! empty( $arrows_position ) ) {
					wp_enqueue_script( 'dipl-blog-slider-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . "includes/modules/BlogSlider/dipl-blog-slider-custom.min.js", array('jquery'), '1.0.1', true );
			...
			if ( 'layout1' === $slider_layout && ! empty( $this->props['post_content_vertical_align'] ) ) {
				self::set_style( $render_slug, array(
					'selector'    => "{$this->main_css_element} .layout1 .dipl_blog_slider_image_wrapper+.dipl_blog_slider_content_wrapper",
					'declaration' => sprintf( 'justify-content: %1$s !important;', esc_attr( $this->props['post_content_vertical_align'] ) ),
				) );
			}
			...
			if ( 'layout2' === $slider_layout ) {
				...
				if ( '' !== $category_background_color ) {
					self::set_style( $render_slug, array(
						'selector'    => "{$this->main_css_element} .dipl_blog_slider_post_categories a",
						'declaration' => sprintf( 'color: %1$s !important;', esc_attr( $category_color ) ),
					) );
				}
			...
			$args = array(
				...
				'backgrounds' => array(
					'post_content_wrapper_background' => array(
		                'normal' => "{$this->main_css_element} .dipl_blog_slider_post",
		                'hover'  => "{$this->main_css_element} .dipl_blog_slider_post:hover",
		            ),
				),
			);
