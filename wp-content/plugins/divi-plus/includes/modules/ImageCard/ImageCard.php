	...
	public function render( $attrs, $content, $render_slug ) {
		...
		if ( isset( $image_wrapper ) || isset( $content_wrapper ) ) {
			...
			if ( 'on' === $this->props['enable_lightbox'] ) {
				...
				wp_enqueue_script( 'dipl-image-card-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . "includes/modules/ImageCard/dipl-image-card-custom.min.js", array('jquery'), '1.0.0', true );

				// lightbox_title_and_caption_style
				array_push( $data_props, 'lightbox_effect', 'lightbox_transition_duration' );
			}

			$data_atts = $this->props_to_html_data_attrs( $data_props );

			$image_card_wrapper = sprintf(
				'<div class="dipl_image_card_wrapper" %3$s>%1$s%2$s</div>',
        ...
        wp_enqueue_style( 'dipl-image-card-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/ImageCard/' . $file . '.min.css', array(), '1.0.0' );
