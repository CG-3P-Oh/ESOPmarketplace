...
class DIPL_TeamGrid extends ET_Builder_Module {
	...
	public function get_fields() {
		$fields = array(
			...
			'post_order' => array(
				...
				'default'          => 'DESC',
				'show_if_not'      => array( 'post_order_by' => 'menu_order' ),
				'tab_slug'         => 'general',
				'toggle_slug'      => 'main_content',
			...
			'use_button' => array(
				'label'            => esc_html__( 'Use Button', 'divi-plus' ),
				'type'             => 'yes_no_button',
				'option_category'  => 'configuration',
				'options'          => array(
					'off' => esc_html__( 'No', 'divi-plus' ),
				...
				'tab_slug'         => 'general',
				'toggle_slug'      => 'elements',
				'description'      => esc_html__( 'Enable this to show button and perform action on it instead of whole item.', 'divi-plus' ),
				'computed_affects' => array(
					'__team_grid_data',
				),
			),
	...
	public static function get_team_members( $attrs = array(), $conditional_tags = array(), $current_page = array() ) {
		...
		if ( ! empty( $post_order ) && 'term_order' !== $post_order_by ) {
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		$use_button				      = esc_attr( $this->props['use_button'] );
		...
		wp_enqueue_script( 'dipl-team-grid-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TeamGrid/dipl-teamgrid-custom.min.js', array('jquery'), '1.0.4', true );
        ...
        wp_enqueue_style( 'dipl-team-grid-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TeamGrid/' . $file . '.min.css', array(), '1.0.0' );
		...
		if ( $query->have_posts() ) {
			...
			if ( 'do_nothing' !== $onclick_trigger && 'on' === $use_button ) {
				$data_props[] = 'button_text';
				// $data_props[] = 'link_button_icon';
				$data_props[] = 'custom_link_button';
			}
            ...
            if ( 'on' === $show_pagination ) {
				...
				$data_atts = $this->props_to_html_data_attrs( $data_props );
				if ( 'do_nothing' !== $onclick_trigger && 'on' === $use_button && ! empty( $this->props['link_button_icon'] ) ) {
					$data_atts .= sprintf( ' data-link_button_icon="%1$s"', et_pb_process_font_icon( $this->props['link_button_icon'] ) );
				}
				...
				$pagination = sprintf(
					'<div class="dipl_team_grid_pagination_wrapper"
						data-total_pages="%1$s"
						data-is_team_member_category="%2$s"
						data-is_product_taxonomy="%3$s"
						data-term_id="%4$s"
						data-term_slug="%5$s"
						data-taxonomy="%6$s"
						data-is_search="%7$s"
						data-search_query="%8$s"
						data-is_user_logged_in="%9$s"
						data-order_class="%10$s"
						data-team_content_padding_top_desktop="%11$s"
						data-team_content_padding_top_tablet="%12$s"
						data-team_content_padding_top_phone="%13$s"
						data-team_content_padding_bottom_desktop="%14$s"
						data-team_content_padding_bottom_tablet="%15$s"
						data-team_content_padding_bottom_phone="%16$s"
						data-popup_elements="%17$s"
					%18$s>
						<ul></ul>
					</div>',
					$total_pages,
			...
			$common_attrs = array();
			if ( 'open_popup' === $onclick_trigger ) {
				$common_attrs[] = 'data-close_icon_position="' . esc_attr( $close_icon_position ) . '" ';
				$common_attrs[] = 'data-popup_name_level="' . esc_attr( $processed_popup_name_level ) .'" ';
				if ( '' !== $display_in_popup ) {
					if ( in_array( 'image', $display_in_popup ) ) {
						$common_attrs[] = 'data-image="on"';
						$common_attrs[] = 'data-image_size="' . esc_attr( $image_size ) . '"';
					}
					...
					if ( in_array( 'skills_bars', $display_in_popup ) ) { 
						...
						if ( 'layout1' === $bar_layout ) {
							...
							if ( 'on' === $use_stripes ) {
								$common_attrs[] = 'data-use_stripes="on"';
								if ( 'on' === $use_animated_stripes ) {
									$common_attrs[] = 'data-use_animated_stripes="on"';
								}	
							}
						} else {
			...
			// $common_attrs[] = 'data-use_button="' . esc_attr( $use_button ) . '"';
			// if ( 'do_nothing' !== $onclick_trigger && 'on' === $use_button ) {
			// 	$common_attrs[] = 'data-button_text="' . esc_html( $button_text ) . '"';
			// 	$common_attrs[] = 'data-button_icon="' . esc_attr( $this->props['button_icon'] ) . '"';
			// }

			$output  .= sprintf('
				<div class="dipl_team_grid_container %1$s" %2$s><div class="dipl_team_grid_items">',
				sanitize_html_class( $select_layout ),
				implode( ' ', $common_attrs )
			);
