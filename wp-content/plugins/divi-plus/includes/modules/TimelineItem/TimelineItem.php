...
class DIPL_Timeline_Item extends ET_Builder_Module {
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		if ( '' !== $single_timeline_icon_color ) {
			ET_Builder_Element::set_style(
				...
				array(
					...
					'declaration' => sprintf( 'color: %1$s !important;', esc_html( $single_timeline_icon_color ) ),
				)
			);
			ET_Builder_Element::set_style(
				$render_slug,
		...
		if ( '' !== $single_timeline_icon_color_hover ) {
			ET_Builder_Element::set_style(
				...
				array(
					...
					'declaration' => sprintf( 'color: %1$s !important;', esc_html( $single_timeline_icon_color_hover ) ),
				)
			);
			ET_Builder_Element::set_style(
				$render_slug,
		...
		if ( 'none' !== $single_icon_shape ) {
			...
			if ( '' !== $single_icon_shape_color ) {
				ET_Builder_Element::set_style(
					...
					array(
						'selector'    => '.dipl_timeline %%order_class%% .dipl_item_icon',
						'declaration' => sprintf( 'background-color: %1$s !important;', esc_html( $single_icon_shape_color ) ),
					)
				);
			}
			if ( isset( $single_icon_shape_color_hover ) ) {
				ET_Builder_Element::set_style(
					...
					array(
						'selector'    => '.dipl_timeline %%order_class%% .dipl_item_icon:hover',
						'declaration' => sprintf( 'background-color: %1$s !important;', esc_html( $single_icon_shape_color_hover ) ),
					)
				);
			}
        ...
        wp_enqueue_style( 'dipl-timeline-item-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TimelineItem/' . $file . '.min.css', array(), '1.0.0' );
