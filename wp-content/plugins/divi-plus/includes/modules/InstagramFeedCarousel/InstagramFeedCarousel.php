...
class DIPL_InstagramFeedCarousel extends ET_Builder_Module {
	...
	public function get_fields() {
		...
		return array(
			...
			'enable_media_height' => array(
				'label'            => esc_html__( 'Enable Image/Video Height', 'divi-plus' ),
				'type'             => 'yes_no_button',
				'option_category'  => 'layout',
				'options'          => array(
					'off' => esc_html__( 'Off', 'divi-plus' ),
				...
				'default_on_front' => 'off',
				'affects'          => array( 'media_height' ),
				'description'      => esc_html__( 'If enabled, an overlay color and icon will be displayed when a visitors hovers over the image', 'divi-plus' ),
				'tab_slug'         => 'advanced',
				'toggle_slug'      => 'post_item',
			),
			'media_height' => array(
				'label'            => esc_html__( 'Image/Video Height', 'divi-plus' ),
				'type'             => 'range',
				...
				'tab_slug'         => 'advanced',
				'toggle_slug'      => 'post_item',
				'description'      => esc_html__( 'Increase or decrease the item height.', 'divi-plus' ),
			),
			'item_bg' => array(
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		if ( ! empty( $accessToken ) ) {
			...
			if ( ! empty( $instagram_feed_data['data'] ) ) {
				...
				wp_enqueue_style( 'dipl-instagram-carousel-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/InstagramFeedCarousel/' . $file . '.min.css', array(), '1.0.0' );
				...
				if ( 'on' === $show_arrow ) {
					...
					if ( ! empty( $arrows_position ) ) {
						wp_enqueue_script( 'dipl-instagram-feed-carousel-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . "includes/modules/InstagramFeedCarousel/dipl-instagram-feed-carousel-custom.min.js", array('jquery'), '1.0.0', true );
				...
				if ( 'on' === $use_overlay ) {
					$this->generate_styles( array(
						...
						'selector'       => '%%order_class%% .dipl_instagram_item-media .et_overlay',
					...
					$this->generate_styles( array(
						...
						'selector'       => '%%order_class%% .dipl_instagram_item-media .et_overlay:before',
						'css_property'   => 'color',
						'render_slug'    => $render_slug,
						'important'      => true,
						'type'           => 'color',
					...
					et_pb_responsive_options()->generate_responsive_css( $overlay_icon_size, '%%order_class%% .dipl_instagram_item-media .et_overlay:before', 'font-size', $render_slug, '', 'range' );
				}

				// Media height.
				if ( 'on' === $this->props['enable_media_height'] ) {
					$media_height = et_pb_responsive_options()->get_property_values( $this->props, 'media_height' );
					if ( ! empty( array_filter( $media_height ) ) ) {
						et_pb_responsive_options()->generate_responsive_css( $media_height, '%%order_class%% .dipl_instagram_item-image, %%order_class%% .dipl_instagram_item-video', 'height', $render_slug, '!important;', 'range' );
						// et_pb_responsive_options()->generate_responsive_css( $media_height, '%%order_class%% .dipl_instagram_item-image, %%order_class%% .dipl_instagram_item-video', 'min-height', $render_slug, '!important;', 'range' );
					}
				}
			} else {
				$instaFeed = sprintf(
