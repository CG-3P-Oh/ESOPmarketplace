...
class DIPL_ImageMagnifier extends ET_Builder_Module {
	...
	public function render( $attrs, $content, $render_slug ) {
        ...
        wp_enqueue_script( 'dipl-image-magnifier-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . "includes/modules/ImageMagnifier/dipl-image-magnifier-custom.min.js", array('jquery'), '1.0.0', true );
        $file = et_is_builder_plugin_active() ? 'style-dbp' : 'style';
		wp_enqueue_style( 'dipl-image-magnifier-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/ImageMagnifier/' . $file . '.min.css', array(), '1.0.0' );
		...
		if ( ! empty( array_filter( $lens_size ) ) ) {
			et_pb_responsive_options()->generate_responsive_css( $lens_size, '%%order_class%% .magnify > .magnify-lens', 'width', $render_slug, '!important;', 'type' );
			et_pb_responsive_options()->generate_responsive_css( $lens_size, '%%order_class%% .magnify > .magnify-lens', 'height', $render_slug, '!important;', 'type' );
		}
		...
		if ( '' !== $lens_border_color && '' !== $lens_border_width ) {
			self::set_style(
				...
				array(
	                ...
	                'declaration' => sprintf(
	                    'box-shadow: 0 0 0 %1$s %2$s, 0 0 %1$s %1$s rgba(0, 0, 0, 0.25), inset 0 0 40px 2px rgba(0, 0, 0, 0.25) !important;',
	                    $lens_border_width,
	                    $lens_border_color
	                )
            ));
