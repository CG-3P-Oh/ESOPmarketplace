...
class DIPL_ImageMask_Item extends ET_Builder_Module {
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		if ( ! empty( array_filter( $element_position_top ) ) ) {
			et_pb_responsive_options()->generate_responsive_css( $element_position_top, '.dipl_image_mask %%order_class%%', 'top', $render_slug, '!important;', 'type' );
		}

		if ( ! empty( array_filter( $element_position_left ) ) ) {
			et_pb_responsive_options()->generate_responsive_css( $element_position_left, '.dipl_image_mask %%order_class%%', 'left', $render_slug, '!important;', 'type' );
		}

		if ( ! empty( array_filter( $element_sizing ) ) ) {
			et_pb_responsive_options()->generate_responsive_css( $element_sizing, '%%order_class%% svg', 'width', $render_slug, '!important;', 'type' );
			et_pb_responsive_options()->generate_responsive_css( $element_sizing, '%%order_class%% svg', 'height', $render_slug, '!important;', 'type' );
		}
        ...
        wp_enqueue_style( 'dipl-image-mask-item-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/ImageMaskItem/' . $file . '.min.css', array(), '1.0.0' );
