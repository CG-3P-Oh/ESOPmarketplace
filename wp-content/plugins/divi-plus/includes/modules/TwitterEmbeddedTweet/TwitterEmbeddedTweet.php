...
class DIPL_TwitterEmbeddedTweet extends ET_Builder_Module {
	...
	public function render( $attrs, $content, $render_slug ) {
    	...
    	if ( '' !== $tweet_id ) {
        	...
        	wp_enqueue_style( 'dipl-twitter-embedded-tweet-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TwitterEmbeddedTweet/' . $file . '.min.css', array(), '1.0.0' );
			$embed_tweet = sprintf(
		...
		self::set_style(
			...
			array(
				'selector'    => '%%order_class%% .dipl_twitter_embedded_tweet_wrapper',
				'declaration' => sprintf( 'width: %1$s !important;', esc_attr( $tweet_max_width ) ),
			)
		);
