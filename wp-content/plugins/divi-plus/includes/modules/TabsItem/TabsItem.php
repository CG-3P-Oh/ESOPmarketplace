...
class DIPL_TabsItem extends ET_Builder_Module {
	...
	public function get_fields() {
		...
		$layouts    = array();
		$layouts[0] = esc_html__( 'Select', 'divi-plus' );
		if ( $library_layouts ) {
	...
	public static function dipl_content_layout( $args = array() ) {
		...
		if( 'dipl_content_layout' === $content_type && '' !== $select_content_layout && 0 !== $select_content_layout ) {
    		...
    		$output .= '<style type="text/css" class="dipl_tabs_layout_styles">' . $internal_style . ' </style>';
		} else {
			$output = '';
		}
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		$fields = array( 'tab_margin_padding' );
		DiviPlusHelper::process_advanced_margin_padding_css( $this, $render_slug, $this->margin_padding, $fields );

		$file = et_is_builder_plugin_active() ? 'style-dbp' : 'style';
        wp_enqueue_style( 'dipl-tabs-item-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TabsItem/' . $file . '.min.css', array(), '1.0.0' );
