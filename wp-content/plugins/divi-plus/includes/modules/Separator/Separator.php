...
class DIPL_Separator extends ET_Builder_Module {
		...
		return array(
			'fonts'                 => array(
				'title' => array(
					...
					'css'             => array(
						'main' => '%%order_class%% .dipl-text-wrapper h1, %%order_class%% .dipl-text-wrapper h2, %%order_class%% .dipl-text-wrapper h3, %%order_class%% .dipl-text-wrapper h4, %%order_class%% .dipl-text-wrapper h5, %%order_class%% .dipl-text-wrapper h6',
						'important' => 'all',
					),
					'important'       => 'all',
					'tab_slug'        => 'advanced',
				...
				'body'  => array(
					...
					'css'             => array(
						'main' => '%%order_class%% .dipl-text-wrapper p, %%order_class%% .dipl-text-wrapper',
						'important' => 'all',
					),
	...
	public function get_fields() {
		...
		return array(
			...
			'separator_image_alt_text' => array(
				...
				'tab_slug'           => 'general',
				'toggle_slug'        => 'main_content',
				'description'        => esc_html__( 'Here you can input the separator image alt text.', 'divi-plus' ),
			),
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		wp_enqueue_script( 'dipl-separator-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . "includes/modules/Separator/dipl-separator-custom.min.js", array('jquery'), '1.0.0', true );
		$file = et_is_builder_plugin_active() ? 'style-dbp' : 'style';
        wp_enqueue_style( 'dipl-separator-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/Separator/' . $file . '.min.css', array(), '1.0.0' );

		if ( 'dipl_line' === $separator_type ) {
			if ( 'off' !== $use_icon_font_size ) {
				$font_size_values = et_pb_responsive_options()->get_property_values( $this->props, 'icon_font_size' );
				et_pb_responsive_options()->generate_responsive_css( $font_size_values, '%%order_class%% .dipl-icon-wrapper .et-pb-icon', 'font-size', $render_slug, '!important;', 'type' );
			}
				...
				if ( '' !== $icon_color ) {
					self::set_style(
						...
						array(
							...
							'declaration' => sprintf( 'color: %1$s !important;', esc_attr( $icon_color ) ),
						)
					);
				}
						...
						if ( 'use_hexagon' === $icon_shape ) {
							self::set_style(
								...
								array(
									...
									'declaration' => sprintf( 'background-color: %1$s !important;', esc_attr( $shape_color ) ),
								)
							);
						} else {
							self::set_style(
						...
						if ( 'use_hexagon' === $icon_shape ) {
							self::set_style(
								...
								array(
									...
									'declaration' => sprintf( 'background-color: %1$s !important;', esc_attr( $shape_color_hover ) ),
								)
							);
						} else {
							self::set_style(
			...
			$separator_image = $multi_view->render_element( array(
				'tag'      => 'img',
				'attrs'    => array(
					'src'   => '{{separator_image}}',
					'class' => 'dipl_separator_image',
					'alt'   => '{{separator_image_alt_text}}'
				),
				'required' => array(
					'separator_type' => 'dipl_line',
					'use_with'       => 'image',
			...
			if ( '' !== $seprator_thickness ) {
				self::set_style(
					...
					array(
						...
						'declaration' => sprintf( 'height: %1$s !important;', esc_attr( $seprator_thickness ) ),
					)
				);
			}
			...
			if ( '' !== $separator_color ) {
				self::set_style(
					...
					array(
						'selector'    => '%%order_class%%.dipl_separator .dipl_shadow',
						'declaration' => 'background: radial-gradient(ellipse at 50% -50% , ' . esc_attr( $separator_color ) . ' 0%, rgba(0, 0, 0, 0) 75%), repeat scroll !important;',
					)
				);
			}
			...
			if ( isset( $separator_color_hover ) ) {
				self::set_style(
					...
					array(
						...
						'declaration' => 'background: radial-gradient(ellipse at 50% -50% , ' . esc_attr( $separator_color_hover ) . ' 0%, rgba(0, 0, 0, 0) 75%), repeat scroll !important;',
					)
				);
			}
