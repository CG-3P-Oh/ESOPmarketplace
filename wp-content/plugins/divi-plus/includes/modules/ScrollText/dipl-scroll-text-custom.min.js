jQuery( document ).ready( function($) {
	if ( $( '.dipl_scroll_text' ).length > 0 ) {
		gsap.registerPlugin( ScrollTrigger );
		$( '.dipl_scroll_text' ).each( function () {

			let $thisObj = $( this ),
				$wrapObj = $thisObj.find( '.dipl-scroll-text-wrap' );

			// Get variables from data attrs.
			let $innerWrap   = $wrapObj.find( '.dipl-scroll-text-inner' ),
			...
			// Add effect class.
			$innerWrap.addClass( `dipl_scroll_${splitType}_${effect}` );
			$innerWrap.html( '' );
			...
			if ( 'letter' === splitType ) {
				...
				words.forEach( ( word, wIdx ) => {
					// Create word span.
					let wordSpan = $( '<span>', { class: 'dipl_st_word' } );

					// Split letters inside word.
					for ( let i = 0; i < word.length; i++ ) {
						let letterSpan = $( '<span>', {
							class: 'dipl_st_letter',
							text: word[i]
						} );
