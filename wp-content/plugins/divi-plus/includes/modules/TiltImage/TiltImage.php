	...
	public function render( $attrs, $content, $render_slug ) {
		...
		if ( 'on' === $use_icon ) {
			...
			if ( ! empty( array_filter( $icon_color ) ) ) {
				et_pb_responsive_options()->generate_responsive_css( $icon_color, '%%order_class%% .dipl_tilt_icon.et-pb-icon', 'color', $render_slug, '!important;', 'color' );
			}
			...
			if ( 'on' === $use_icon_font_size ) {
				if ( ! empty( array_filter( $icon_font_size ) ) ) {
					et_pb_responsive_options()->generate_responsive_css( $icon_font_size, '%%order_class%% .dipl_tilt_icon.et-pb-icon', 'font-size', $render_slug, '!important;', 'font' );
				}
			}
		}
		...
		if ( 'on' === $use_overlay ) {
			if ( ! empty( array_filter( $overlay_color ) ) ) {
				et_pb_responsive_options()->generate_responsive_css( $overlay_color, '%%order_class%% .dipl_tilt_image_inner_wrapper:before', 'background-color', $render_slug, '!important;', 'background-color' );
			}
		...
		wp_enqueue_script( 'dipl-tilt-image-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . "includes/modules/TiltImage/dipl-tilt-image-custom.min.js", array('jquery'), '1.0.0', true );
		$file = et_is_builder_plugin_active() ? 'style-dbp' : 'style';
        wp_enqueue_style( 'dipl-tilt-image-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TiltImage/' . $file . '.min.css', array(), '1.0.0' );

		$fields = array( 'tilt_content_margin_padding' );
