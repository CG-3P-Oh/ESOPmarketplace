...
class DIPL_ImageCardCarouselItem extends ET_Builder_Module {
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		$image = $multi_view->render_element( array(
			...
			'attrs'    => array(
				'src'   => '{{image}}',
				'class' => 'dipl_image_card_image no-lazyload skip-lazy',
				'alt'   => $image_alt,
			),
			'required' => 'image',
		) );
		...
		$title = ''; 
		if ( ! isset( $dp_icc_parent_attrs['hide_title'] ) || 'off' === $dp_icc_parent_attrs['hide_title'] ) {
		...
		if ( '' !== $icon ) {
            ...
            /* Add margin to icon */
			et_pb_responsive_options()->generate_responsive_css( $margin_values, '%%order_class%% .dipl_image_card_image_wrapper + .dipl_image_card_content_wrapper .dipl_image_card_icon_wrapper', 'margin-top', $render_slug, '!important;', 'range' );
			et_pb_responsive_options()->generate_responsive_css( $content_margin_values, '%%order_class%% .dipl_image_card_inner_content_wrapper', 'margin-top', $render_slug, '!important;', 'range' );
		...
		wp_enqueue_style( 'dipl-image-card-carousel-item-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/ImageCardCarouselItem/' . $file . '.min.css', array(), '1.0.0' );
