...
class DIPL_TestimonialGrid extends ET_Builder_Module {
		...
		return array_merge(
			array(
				...
				'column_spacing' => array(
					...
					'tab_slug'         => 'general',
					'toggle_slug'      => 'display_setting',
					'description'      => esc_html__( 'Here you can choose spacing between testimonial items.', 'divi-plus' ),
				),
	            'use_masonry' => array(
	...
	public static function get_testimonial_data( $args = array(), $conditional_tags = array(), $current_page = array() ) {
		...
		if ( $query->have_posts() ) {
			...
			if ( 'on' === $use_masonry ) {
				...
				$testimonials .= '<div class="dipl_testimonial_isotope_item_gutter"></div>';
			}
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		$testimonial_number = ( 0 === $testimonial_number ) ? -1 : (int) $testimonial_number;
		wp_enqueue_script( 'dipl-testimonial-grid-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TestimonialGrid/dipl-testimonial-grid-custom.min.js', array('jquery','elicus-images-loaded-script'), '1.0.1', true );
        ...
        wp_enqueue_style( 'dipl-testimonial-grid-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TestimonialGrid/' . $file . '.min.css', array(), '1.0.0' );
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				if ( 'on' === $show_author_company_name ) {
					...
					if ( '' !== $company_url && '' !== $company_name ) {
						...
						$author_company = sprintf(
							'<div class="dipl_testimonial_author_company">%1$s</div>',
							esc_html( $company_name )
						);
					}
				}
			...
			et_pb_responsive_options()->generate_responsive_css( $width, '%%order_class%% .dipl_testimonial_layout .dipl_testimonial_isotope_item', 'width', $render_slug, '!important;', 'range' );
			et_pb_responsive_options()->generate_responsive_css( $column_spacing, '%%order_class%% .dipl_testimonial_layout .dipl_testimonial_isotope_item', array( 'margin-bottom' ), $render_slug, '!important;', 'range' );

			if ( 'on' === $use_masonry ) {
				et_pb_responsive_options()->generate_responsive_css( $column_spacing, '%%order_class%% .dipl_testimonial_isotope_item_gutter', 'width', $render_slug, '!important;', 'range' );
			} else {
				foreach ( $number_of_columns as $device => $cols ) {
					if ( 'desktop' === $device ) {
						self::set_style( $render_slug, array(
							'selector'    => '%%order_class%% .dipl_testimonial_layout .dipl_testimonial_isotope_item:not(:nth-child(' . absint( $cols ) . 'n+' . absint( $cols ) . '))',
							'declaration' => sprintf( 'margin-right: %1$s;', esc_attr( $column_spacing['desktop'] ) ),
							'media_query' => self::get_media_query( 'min_width_981' ),
						) );
						if ( '' !== $cols ) {
							self::set_style( $render_slug, array(
								'selector'    => '%%order_class%% .dipl_testimonial_layout .dipl_testimonial_isotope_item:nth-child(' . absint( $cols ) . 'n+1)',
