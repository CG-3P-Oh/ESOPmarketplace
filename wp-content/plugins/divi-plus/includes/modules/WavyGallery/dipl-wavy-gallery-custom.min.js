jQuery( document ).ready( function($) {
	if ( $( '.dipl_wavy_gallery' ).length > 0 ) {
		$( '.dipl_wavy_gallery' ).each( function(e) {
			dipl_init_wavy_gallery( $( this ) );
		} );
	}
} ); // over document ready.

// Init wavy gallery.
function dipl_init_wavy_gallery( thisObj ) {
	gsap.registerPlugin( ScrollTrigger );
	ScrollTrigger.normalizeScroll( true );

	const orderClass = thisObj.prop('class').match(/(dipl_wavy_gallery\_[^\s]*)/)[0] + '_lightbox';

	let $section         = thisObj.find( '.dipl_wavy_gallery_wrapper' ),
	...
	gsap.to( wrapperEl, {
		...
		scrollTrigger: {
			...
			scrub: true,
			pin: true,
			onEnter: () => jQuery('html').addClass( "dipl_hide_scrollbar" ),
			onLeave: () => jQuery('html').removeClass( "dipl_hide_scrollbar" ),
			onEnterBack: () => jQuery('html').addClass( "dipl_hide_scrollbar" ),
			onLeaveBack: () => jQuery('html').removeClass( "dipl_hide_scrollbar" ),
			onUpdate: (self) => {
				isScrolling = true;
	...
	const $overlay = jQuery( '<div>', { class: 'dipl_wavy_gallery_overlay ' + orderClass } )
		.append( jQuery( '<div>', { class: 'dipl_wavy_gallery_overlay_items' } ) )
		.append( jQuery( '<div>', { class: 'dipl_wavy_gallery_overlay_item_title' } ) );

	jQuery('body').append( $overlay );

	let $overlayContent = $overlay.find( '.dipl_wavy_gallery_overlay_items' );
	let $overlayTitle   = $overlay.find( '.dipl_wavy_gallery_overlay_item_title' );	

	// Animate gallery.
	function dipl_wavy_gallery_animate() {
		...
		$images.each( function() {
			...
			if ( inZone && isScrolling ) {
				...
				data.scale = dipl_lerp( data.scale, targetScale, 0.1 );
				...
				data.scale = dipl_lerp( data.scale, 1, 0.1 );
		...
		requestAnimationFrame( dipl_wavy_gallery_animate );
	}
	dipl_wavy_gallery_animate();

	// Update overlay title on click.
	function updateOverlayTitle() {
		if ( 'off' === showOverlayTitle ) {
			$overlayTitle.hide();
			return;
		}
...
function dipl_lerp( start, end, amt ) {
