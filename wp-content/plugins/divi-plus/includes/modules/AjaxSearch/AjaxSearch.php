...
class DIPL_AjaxSearch extends ET_Builder_Module {
		...
		return array_merge(
			array(
				...
				'exclude_protected_posts' => array(
					'label'           => esc_html__( 'Exclude Protected Posts', 'divi-ajax-search' ),
					'type'            => 'yes_no_button',
					'option_category' => 'configuration',
					'options'         => array(
					...
					'tab_slug'        => 'general',
					'toggle_slug'     => 'search_area',
					'description'     => esc_html__( 'Enable to exclude password protected posts.', 'divi-ajax-search' ),
				),
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		wp_enqueue_script( 'dipl-ajax-search-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/AjaxSearch/dipl-ajax-search-custom.min.js', array('jquery'), '1.0.1', true );
		$file = et_is_builder_plugin_active() ? 'style-dbp' : 'style';
        wp_enqueue_style( 'dipl-ajax-search-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/AjaxSearch/' . $file . '.min.css', array(), '1.0.0' );
		...
		if ( '1' !== $number_of_columns && 'on' === $use_masonry ) {
			wp_enqueue_script( 'elicus-isotope-script', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/assets/js/isotope.pkgd.min.js', array( 'jquery' ), '3.0.6', true );
		}

		$search_field_wrap  = sprintf(
			'<div class="dipl_ajax_search_wrap">
				<div class="dipl_ajax_search_field_wrap %6$s">
					<input type="search" placeholder="%1$s" class="dipl_ajax_search_field"
						...
						data-orderby="%10$s" data-order="%11$s" data-use-masonry="%13$s"
						data-exclude_protected_posts="%15$s"
					/>
			...
			esc_attr( $this->props['exclude_protected_posts'] )
		);
