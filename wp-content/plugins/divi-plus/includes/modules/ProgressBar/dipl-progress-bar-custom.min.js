jQuery( document ).ready( function($) {
	if ( $( '.dipl_progress_bar' ).length > 0 ) {
		$( window ).on( 'scroll', diplInitUpdateProgressBars );
		diplInitUpdateProgressBars();
	}
} );

// Init update progress bar.
function diplInitUpdateProgressBars() {
	jQuery( '.dipl_progress_bar' ).each( function() {

		let thisObj = jQuery( this ),
			wrapObj = thisObj.find( '.dipl_progress_bar_wrapper' ),
			$bar    = thisObj.find( '.dipl_progress_bar_inner' );
		...
		if ( wrapObj.hasClass( 'dipl_progress_bar_layout_circle' ) ) {
			// Circle.
			let $circle     = $bar.find( 'circle.dipl_circle_fg' ),
			...
			$bar.find( '.dipl_progress_bar_percent' ).text( Math.round(percent) + '%' );
		} else if ( wrapObj.hasClass( 'dipl_progress_bar_layout_half_circle' ) ) {
			// Half-Circle.
			let $fgPath     = $bar.find( 'path.dipl_circle_fg' ),
				direction   = 'clockwise', // or anticlockwise.
