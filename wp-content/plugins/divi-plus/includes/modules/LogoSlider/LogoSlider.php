...
class DIPL_LogoSlider extends ET_Builder_Module {
	...
	public function render( $attrs, $content, $render_slug ) {
        ...
        wp_enqueue_style( 'dipl-logo-slider-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/LogoSlider/' . $file . '.min.css', array(), '1.0.0' );
		...
		if ( '' !== $slide_image_width ) {
			self::set_style(
				...
				array(
					...
					'declaration' => sprintf( 'width: %1$s !important;', esc_attr( $slide_image_width ) ),
				)
			);
		}
		...
		if ( '' !== $slide_image_height ) {
			self::set_style(
				...
				array(
					...
					'declaration' => sprintf( 'min-height: %1$s !important;', esc_attr( $slide_image_height ) ),
				)
			);
		}
		...
		if ( 'on' === $show_arrow ) {
			...
			if ( ! empty( $arrows_position ) ) {
				wp_enqueue_script( 'dipl-logo-slider-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . "includes/modules/LogoSlider/dipl-logo-slider-custom.min.js", array('jquery'), '1.0.1', true );
