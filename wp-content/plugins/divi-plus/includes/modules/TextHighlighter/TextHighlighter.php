...
class DIPL_Text_Highlighter extends ET_Builder_Module {
	...
	public function get_advanced_fields_config() {
		return array(
			...
			'text'           => false,
			'text_highlighter_spacing' => array(
				'main_text' => array(
					'margin_padding' => array(
						'css' => array(
							'use_padding' => false,
							'margin'      => "{$this->main_css_element} .dipl_text_highlighted_content",
							'important'   => 'all',
						),
					),
				),
	...
	public function get_fields() {
		...
		return array(
			...
			'main_text_custom_margin' => array(
				'label'            => esc_html__( 'Main Text Margin', 'divi-plus' ),
				'type'             => 'custom_margin',
				'option_category'  => 'layout',
				'mobile_options'   => true,
				'hover'            => false,
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		wp_enqueue_script( 'dipl-text-highlighter-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . "includes/modules/TextHighlighter/dipl-text-highlighter-custom.min.js", array('jquery'), '1.0.0', true );
		$file = et_is_builder_plugin_active() ? 'style-dbp' : 'style';
        wp_enqueue_style( 'dipl-text-highlighter-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TextHighlighter/' . $file . '.min.css', array(), '1.0.0' );
		...
		if ( '' !== $highlighter_content ) {
			...
			if ( '' !== $stroke_width ) {
				self::set_style(
					...
					array(
						'selector'    => '%%order_class%% .dipl_text_highlighter_wrapper svg path',
						'declaration' => sprintf( 'stroke-width: %1$s !important;', $stroke_width ),
					)
				);
			}
		...
		$fields = array( 'text_highlighter_spacing' );
