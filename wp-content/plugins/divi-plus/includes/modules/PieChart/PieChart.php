 ...
 * @version    1.20.0
 */
class DIPL_PieChart extends ET_Builder_Module {
	public $slug       = 'dipl_pie_chart';
	public $child_slug = 'dipl_pie_chart_item';
	...
	public function init() {
		$this->name 			= esc_html__( 'DP Pie Chart', 'divi-plus' );
		$this->child_item_text  = esc_html__( 'Pie Chart Item', 'divi-plus' );
	...
	public function get_settings_modal_toggles() {
		return array(
			'advanced' => array(
				'toggles' => array(
					'pie_chart' => esc_html__( 'Pie Chart', 'divi-plus' )
				)
			)
		);
	}
	...
	public function get_fields() {
		return array(
			'chart_height' => array(
				'label'            => esc_html__( 'Pie Chart Height', 'divi-plus' ),
				'type'             => 'range',
			...
			'chart_border_size' => array(
				'label'            => esc_html__( 'Pie Border Size', 'divi-plus' ),
				'type'             => 'range',
				'option_category'  => 'font_option',
				'range_settings'   => array(
					'min'  => '0',
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		$render_output = '';
		if ( ! empty( $dipl_pie_chart_item_data ) ) {

			// Load style and script files.
			wp_enqueue_script( 'elicus-chartjs-script' );
			wp_enqueue_script( 'dipl-pie-chart-script', ELICUS_DIVI_PLUS_PLUGIN_URL . "includes/modules/PieChart/dipl-pie-chart.min.js", array('jquery'), '1.0.0', true );

			$labels      = [];
			$values      = [];
			$bgColor     = [];
			$borderColor = [];
			foreach ( $dipl_pie_chart_item_data as $child ) {
				if ( $child['title'] && $child['value'] ) {
					$labels[]      = esc_html__( $child['title'], 'divi-plus' );
					$values[]      = esc_attr( $child['value'] );
					$bgColor[]     = esc_attr( $child['pie_bg_color'] ) ?? 'rgba(0,0,0,0.2)';
					$borderColor[] = esc_attr( $child['pie_border_color'] ) ?? 'rgb(0,0,0)';
				}
			}
			...
			$render_output = sprintf(
				...
				htmlspecialchars(wp_json_encode( $chart_data ), ENT_QUOTES, 'UTF-8'),
