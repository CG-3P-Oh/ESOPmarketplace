...
class DIPL_TeamSlider extends ET_Builder_Module {
	...
	public function get_advanced_fields_config() {
		return array(
			...
			'button' => array(
				'link_button' => array(
					'label' => esc_html__( 'Link Button', 'divi-plus' ),
					'css'   => array(
						'main'      => "{$this->main_css_element} .dipl-team-item-link-btn-wrap .dipl-team-item-link-btn",
						'alignment' => "{$this->main_css_element} .dipl-team-item-link-btn-wrap",
						'important' => 'all',
					),
					'margin_padding'  => array(
						'css' => array(
							'margin'    => "{$this->main_css_element} .dipl-team-item-link-btn-wrap",
							'padding'   => "{$this->main_css_element} .dipl-team-item-link-btn-wrap .dipl-team-item-link-btn",
							'important' => 'all',
						),
					),
					'no_rel_attr'     => true,
			...
			'borders' => array(
				...
				'bars_border' => array(
					...
					'depends_on'      => array( 'popup_bar_layout' ),
	...
	public function get_fields() {
		$team_slider_fields = array(
			...
			'link_type' => array(
                'label'             => esc_html__( 'Link Type', 'divi-plus' ),
                'type'              => 'select',
                'option_category'   => 'configuration',
                'options'           => array(
                    'item'   => esc_html__( 'Link Whole Item', 'divi-plus' ),
                    'button' => esc_html__( 'Show Button', 'divi-plus' ),
                ),
                'default'           => 'item',
                'show_if'			=> array(
                ...
                'tab_slug'          => 'general',
                'toggle_slug'       => 'link_options',
                'description'       => esc_html__( 'Here you can choose whether to link the whole team member item or use the button.', 'divi-plus' ),
                'computed_affects'  => array(
					'__team_slider_data',
				),
            ),
			'link_button_text' => array(
				'label'            => esc_html__( 'Link Button Text', 'divi-plus' ),
				'type'             => 'text',
				'option_category'  => 'configuration',
			...
			'use_popup' => array(
				'label'            => esc_html__( 'Use Popup', 'divi-plus' ),
				'type'             => 'yes_no_button',
				'option_category'  => 'configuration',
				'options'          => array(
					'on'  => esc_html__( 'Yes', 'divi-plus' ),
				...
				'tab_slug'         => 'general',
				'toggle_slug'      => 'link_options',
				'description'      => esc_html__( 'Enable this to open popup/lightbox instead of open a link.', 'divi-plus' ),
				'computed_affects' => array(
					'__team_slider_data',
				),
			),
			...
			'popup_use_animated_stripes' => array(
	...
	public static function get_team_members( $attrs = array(), $conditional_tags = array(), $current_page = array() ) {
		...
		global $wp_filter, $et_fb_processing_shortcode_object, $et_pb_rendering_column_content;
		...
		$defaults = array(
			...
			'link_button_icon'        => '',
			'link_button_text'        => esc_html__( 'Read more', 'divi-plus' )
		);
		...
		if ( $query->have_posts() ) {
			...
			$wrapper_class = array( 'dipl_team_member_card' );
			if ( 'on' === $enable_member_link && 'item' === $link_type ) {
				if (  'on' === $use_popup ) {
			...
			while ( $query->have_posts() ) {
					...
					for ( $i = 0; $i < count( $team_skills ); $i++ ) {
						...
						$skill_bar .= sprintf(
							'<div class="dipl_skill_bar_wrapper_inner">
								<div class="dipl_skill_name">%1$s</div>
				...
				$wrap_data_attrs = array();
				if ( 'on' === $enable_member_link && 'item' === $link_type ) {
					$wrap_data_attrs[] = sprintf( 'data-link="%1$s"', esc_url( get_permalink( $post_id ) ) );
					$wrap_data_attrs[] = sprintf( 'data-link_target="%1$s"', ( 'on' === $link_target ) ? '_blank' : '_self' );
				}

				$readmore_button = '';
				if ( 'on' === $enable_member_link && 'button' === $link_type ) {
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		$link_type				      = esc_attr( $this->props['link_type'] );
		$use_popup                    = esc_attr( $this->props['use_popup'] );
		...
		$display_in_popup             = esc_attr( $this->props['display_in_popup'] );
		$popup_width       			  = esc_attr( $this->props['popup_width'] );
		$popup_bg       			  = esc_attr( $this->props['popup_bg'] );
		$popup_bar_layout             = esc_attr( $this->props['popup_bar_layout'] );
		$popup_use_stripes            = esc_attr( $this->props['popup_use_stripes'] );
		$popup_animation_speed        = esc_attr( $this->props['popup_animation_speed'] );
		$popup_use_animated_stripes   = esc_attr( $this->props['popup_use_animated_stripes'] );
		$popup_stripe_color           = esc_attr( $this->props['popup_stripe_color'] );
		$popup_social_icon_color      = esc_attr( $this->props['popup_social_icon_color'] );
		$popup_social_icon_bg_color   = esc_attr( $this->props['popup_social_icon_background_color'] );
		...
		$popup_skill_bar_height       = et_pb_responsive_options()->get_property_values( $this->props, 'popup_skill_bar_height' );
		...
		if ( $query->have_posts() ) {
			...
			wp_enqueue_script( 'dipl-team-slider-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TeamSlider/dipl-team-slider-custom.min.js', array('jquery'), '1.0.3', true );
			wp_localize_script( 'dipl-team-slider-custom', 'DiviPlusTeamSliderData', array(
	            'ajaxurl'   => admin_url( 'admin-ajax.php' ),
	            'ajaxnonce' => wp_create_nonce( 'elicus-divi-plus-team-grid-nonce' ),
	        ) );

			$file = et_is_builder_plugin_active() ? 'style-dbp' : 'style';
        	wp_enqueue_style( 'dipl-team-slider-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TeamSlider/' . $file . '.min.css', array(), '1.0.0' );

			if ( 'on' === $enable_member_link && 'on' === $use_popup ) {
			...
			$common_atts = array();
			$common_atts[] = 'data-close_icon_position="' . $close_icon_position . '"';
			$common_atts[] = 'data-popup_name_level="' . $processed_popup_name_level .'"';
			if ( '' !== $display_in_popup ) {
				if ( in_array( 'image', $display_in_popup ) ) {
					$common_atts[] = 'data-image="on"';
					// $common_atts[] = 'data-image_size="'.$image_size.'"';
				}
				if ( in_array( 'designation', $display_in_popup ) ) { 
					$common_atts[] = 'data-designation="on"';
				}
				if ( in_array( 'content', $display_in_popup ) ) { 
					$common_atts[] = 'data-content="on"';	
				}
				if ( in_array( 'skills_bars', $display_in_popup ) ) { 
					$common_atts[] = 'data-skills_bars="on"';
					if ( 'layout1' === $popup_bar_layout ) {
						$common_atts[] = 'data-bar_layout="1"';
						if ( 'on' === $popup_use_stripes ) {
							$common_atts[] = 'data-use_stripes="on"';
							if ( 'on' === $popup_use_animated_stripes ) {
								$common_atts[] = 'data-use_animated_stripes="on"';
							}	
						}
					} else {
						$common_atts[] = 'data-bar_layout="2"';
					}
				}
				if ( in_array( 'social_icons', $display_in_popup ) ) { 
					$common_atts[] = 'data-social_icons="on"';	
				}
			}
			...
			while ( $query->have_posts() ) {
				...
				$wrap_data_attrs = $common_atts;
				if ( 'on' === $enable_member_link ) {
					if ( 'on' === $use_popup ) {
						$wrap_data_attrs[] = sprintf( 'data-id="%1$s"', esc_attr( $post_id ) );
				...
				$readmore_button = '';
				if ( 'on' === $enable_member_link && 'button' === $link_type ) {
					$link_button_text = ! empty( $this->props['link_button_text'] ) ? esc_attr( $this->props['link_button_text'] ) : '';
					$button_class = array( 'dipl-team-item-link-btn' );
					$button_link  = esc_url( get_permalink( $post_id ) );
					if ( 'on' === $use_popup ) {
						$button_class[] = 'dipl_team_popup';
					...
					// $this->render_button, is not adding button class.
					$readmore_button = dipl_render_divi_button( array(
						'button_text'         => esc_attr( $link_button_text ),
						'button_text_escaped' => true,
						'has_wrapper'      	  => false,
						...
						'button_url'          => $button_link,
						'url_new_window'      => esc_attr( $link_target ),
						'button_custom'       => isset( $this->props['custom_link_button'] ) ? esc_attr( $this->props['custom_link_button'] ) : 'off',
						'custom_icon'         => isset( $this->props['link_button_icon'] ) ? $this->props['link_button_icon'] : '',
						'button_rel'          => isset( $this->props['link_button_rel'] ) ? esc_attr( $this->props['link_button_rel'] ) : '',
					) );
					if ( ! empty( $readmore_button ) ) {
						$readmore_button = sprintf(
							'<div class="et_pb_button_wrapper dipl-team-item-link-btn-wrap">%1$s</div>',
			...
			if ( 'on' === $enable_member_link && 'on' === $use_popup ) {
				self::set_style( $render_slug, array(
					'selector'    => '%%order_class%% .dipl_team_member_slide',
					'declaration' => 'cursor: pointer;',
				) );

				// Social icon.
				if ( ! empty( $display_in_popup ) && in_array( 'social_icons', $display_in_popup ) ) {
					if ( 'dipl_icon_center' === $popup_social_icon_alignment ) {
						self::set_style( $render_slug, array(
							'selector'    => '%%order_class%%_lightbox .dipl_team_social_wrapper',
							'declaration' => 'justify-content: center;',
						) );
					} elseif ( 'dipl_icon_left' === $popup_social_icon_alignment ) {
						self::set_style( $render_slug, array(
							'selector'    => '%%order_class%%_lightbox .dipl_team_social_wrapper',
							'declaration' => 'justify-content: flex-start;',
						) );
					} elseif ( 'dipl_icon_right' === $popup_social_icon_alignment ) {
						self::set_style( $render_slug, array(
							'selector'    => '%%order_class%%_lightbox .dipl_team_social_wrapper',
							'declaration' => 'justify-content: flex-end;',
						) );
					...
					if (  '' !== $popup_social_icon_color || '' !== $popup_social_icon_bg_color ) {
						self::set_style( $render_slug, array(
				...
				// Skills bars.
				if ( ! empty( $display_in_popup ) && in_array( 'skills_bars', $display_in_popup ) ) {
					if ( 'layout2' === $popup_bar_layout ) {
						if ( ! empty( array_filter( $popup_skill_bar_height ) ) ) {
							et_pb_responsive_options()->generate_responsive_css( $popup_skill_bar_height, '%%order_class%%_lightbox .dipl_skill_bar_wrapper .dipl_bar_counter_chunks', 'height', $render_slug, '!important;', 'range' );
						}
						$this->generate_styles( array(
							'base_attr_name' => 'popup_empty_bar_color',
						...
						$this->generate_styles( array(
							'base_attr_name' => 'popup_filled_bar_color',
						...
						if ( ! empty( array_filter( $popup_skill_bar_height ) ) ) {
							et_pb_responsive_options()->generate_responsive_css( $popup_skill_bar_height, '%%order_class%%_lightbox .dipl_skill_bar_wrapper .dipl_empty_bar', 'height', $render_slug, '!important;', 'range' );
						}
						$this->generate_styles( array(
							'base_attr_name' => 'popup_empty_bar_color',
					...
					if ( '' !== $popup_animation_speed ) {
					    self::set_style( $render_slug, array(
							'selector'    => '%%order_class%%_lightbox .dipl_bar_counter_animated_striped_bar:before',
							'declaration' => sprintf( 'animation-duration: %1$ss;', esc_attr( $popup_animation_speed ) ),
						) );
					}
					if ( '' !== $popup_stripe_color ) {
					    self::set_style( $render_slug, array(
	...
	public function dipl_render_slider_script() {
		...
		if ( in_array( $slide_effect, array( 'slide', 'coverflow' ), true ) ) {
			...
			if ( $post_per_view > $slidesPerGroup && 1 !== $slidesPerGroup ) {
				$slidesPerGroupSkip = intval( $post_per_view ) - intval( $slidesPerGroup );
			}
			if ( $post_per_view_ipad > $slidesPerGroupIpad && 1 !== $slidesPerGroupIpad ) {
				$slidesPerGroupSkipIpad = intval( $post_per_view_ipad ) - intval( $slidesPerGroupIpad );
			}
			if ( $post_per_view_mobile > $slidesPerGroupMobile && 1 !== $slidesPerGroupMobile ) {
				$slidesPerGroupSkipMobile = intval( $post_per_view_mobile ) - intval( $slidesPerGroupMobile );
			}
		} else {
			$post_per_view        		= 1;
