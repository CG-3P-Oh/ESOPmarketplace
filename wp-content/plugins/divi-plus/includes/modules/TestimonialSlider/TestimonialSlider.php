...
class DIPL_TestimonialSlider extends ET_Builder_Module {
		...
		return array_merge(
			array(
				...
				'auto_height_slider' => array(
					...
					'show_if'          => array(
						'equalize_testimonials_height' => 'off',
					),
					'default'         => 'off',
					'tab_slug'        => 'general',
					'toggle_slug'     => 'slider_settings',
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		if ( $query->have_posts() ) {
        	...
        	wp_enqueue_style( 'dipl-testimonial-slider-style', ELICUS_DIVI_PLUS_PLUGIN_URL . 'includes/modules/TestimonialSlider/' . $file . '.min.css', array(), '1.0.0' );
			...
			if ( 'on' === $show_arrow ) {
				...
				if ( ! empty( $arrows_position ) ) {
					wp_enqueue_script( 'dipl-testimonial-slider-custom', ELICUS_DIVI_PLUS_PLUGIN_URL . "includes/modules/TestimonialSlider/dipl-testimonial-slider-custom.min.js", array('jquery'), '1.0.2', true );
	...
	public function dipl_render_slider_script() {
		...
		$equalize_height            = esc_attr( $this->props['equalize_testimonials_height'] );
		...
		$auto_height_slider			= 'on' === $equalize_height ? 'false' : $auto_height_slider;
		$arrows 					= 'false';
		$dots 						= 'false';
		$autoplaySlides				= 0;
		$cube 						= 'false';
